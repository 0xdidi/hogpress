...
namespace Hogpress\Platform\Connection;
...
final class Validator {
	...
	public static function validate( $api_key, $host ) {
		...
		if ( '' === $api_key ) {
			return array(
				'ok'      => false,
				'message' => __( 'Enter your PostHog project API key.', 'hogpress' ),
			);
		}
		...
		if ( ! Host::is_valid_url( $host ) ) {
			return array(
				'ok'      => false,
				'message' => __( 'That host does not look like a valid URL. Check the region or custom host.', 'hogpress' ),
			);
		}
		...
		$response = wp_remote_post(
			...
			array(
				...
				'body'        => wp_json_encode(
					array(
						'api_key'     => $api_key,
						'distinct_id' => 'hogpress-setup-check',
					)
				),
			)
		);
		...
		if ( is_wp_error( $response ) ) {
			return array(
				...
				'message' => sprintf( __( 'We could not reach PostHog at %s. Check the region or custom host and try again.', 'hogpress' ), $host ),
			);
		}
		...
		if ( 200 === $code ) {
			return array(
				'ok'      => true,
				'message' => __( 'Connected. Your key and host check out.', 'hogpress' ),
			);
		}
		...
		if ( 401 === $code ) {
			return array(
				'ok'      => false,
				'message' => __( 'That project API key was rejected by PostHog. Double-check the key and the region.', 'hogpress' ),
			);
		}
		...
		return array(
			...
			'message' => sprintf( __( 'PostHog returned an unexpected response (HTTP %d). Try again in a moment.', 'hogpress' ), $code ),
		);
	}
}
